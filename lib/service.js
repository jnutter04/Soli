...
/* When a service was recorded, which is not when it happened.

   `date` is the appointment, and people backdate it: they catch up on Sunday
   for a week of work, or import a year from a booking app in one go. So `date`
   cannot answer how Soli actually fits into somebody's day, whether they log
   between clients or in a weekly sitting, or how long a new account takes to
   record its first service. That last one decides how much of the trial is
   even in play.

   Only set where a person recorded something. The sample data deliberately
   goes unstamped, since loading examples is not a logging event, which makes
   the presence of this field a clean marker of real work.

   Existing services predate this and will not have it. It only accrues from
   here, which is the argument for adding it before it is needed rather than
   when the question comes up. */
export const newLog = (fields) => ({
  ...fields,
